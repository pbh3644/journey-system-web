...
public class RsaUtils {
    ...
    private static final char LOWERC_ASE_A = 'a';
    private static final char CAPITAL_F = 'F';
    private static final char LOWERC_ASE_F = 'f';
    ...
    public static byte[] asciiToBcd(byte[] ascii, int ascLen) {
        byte[] bcd = new byte[ascLen / TWO];
        int j = 0;
        for (int i = 0; i < (ascLen + 1) / TWO; i++) {
    ...
    public static byte ascToBcd(byte asc) {
        byte bcd;
        if ((asc >= ZERO) && (asc <= NINE)) {
            bcd = (byte) (asc - ZERO);
        } else if ((asc >= CAPITAL_A) && (asc <= CAPITAL_F)) {
            bcd = (byte) (asc - CAPITAL_A + 10);
        } else if ((asc >= LOWERC_ASE_A) && (asc <= LOWERC_ASE_F)) {
            bcd = (byte) (asc - LOWERC_ASE_A + 10);
        } else {
            bcd = (byte) (asc - 48);
        }
        return bcd;
    ...
    public static void main(String[] args) throws Exception {
        ...
        String name = encryptByPublicKey("pangbohuan");
